...
const double HOME_LNG = 74.878120;                          // to track how far away the "Dog" is away from Home 

/************************* WiFi Access Point *********************************/

#define WLAN_SSID       "Razzberry"                          // Enter Your router SSID
#define WLAN_PASS       "monkeyberry1"                       // Enter Your router Password
...
#define AIO_KEY          "70f44171d5fd4a7597dd201e9fe2fb17"
...
void loop() {
  ...
  float Speed_Bus;                               // variable to store Distance to Home  
  ...
  Speed_Bus = gps.speed.kmph();                      //(unsigned long)TinyGPSPlus::distanceBetween(gps.location.lat(),gps.location.lng(),HOME_LAT, HOME_LNG);  //Query Tiny GPS to Calculate Distance to Home
...
//  display.update();      
  ...
  Serial.print("\nSpeed: ");
  Serial.print(Speed_Bus);                    // Distance to Home measured in Meters    
 
  if  (Speed_Bus > 80)
      {
        Serial.print("Alert! OverSpeeding");
      }
